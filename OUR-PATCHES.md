...
2. `managers/transcription.rs`:
   - `transcribe_groq_wav(wav_path)` — публичный метод: берёт API ключ из настроек, вызывает `transcribe_via_groq_file`, **уважает `settings.post_process_enabled`** (если выкл — пропускает `llm_post_process`), прогоняет через фильтры, добавляет абзацы
   ...
   **Важно:** раньше `transcribe_groq_wav` вызывал `llm_post_process` **безусловно**, из-за чего LLM-постпроцессинг работал всегда для Groq-ветки, даже если в UI тумблер выключен. Симптомы: в результат иногда просачиваются мета-комментарии Groq LLM («это не совсем корректная фраза. Исправленная транскрипция: …») или целые предложения подменяются на «улучшенные» варианты. Сейчас путь через пост-процессинг идёт только при `post_process_enabled = true`.

### Чанкинг WAV для предотвращения обрезки

`whisper-large-v3` имеет внутреннее 30-секундное окно обработки. Записи ~25-60+ секунд стабильно теряют конец — Whisper обрезает на границе окна. **Retry целого файла не помогает** — обе попытки дают одинаково обрезанный результат в одном месте.

Решение: `transcribe_groq_wav_in_chunks` всегда режет WAV на куски по **25 секунд** через `hound` (чистый Rust, без ffmpeg). Каждый кусок отправляется в Groq отдельно, результаты склеиваются через пробел. Последний кусок может быть короче 25 секунд — Whisper нормально транскрибирует короткие клипы.

Дополнительно: если отдельный чанк вернул явную галлюцинацию (весь текст = "и так далее." / "и т.д." / субтитры / слишком мало символов на секунду аудио) — он перезапрашивается один раз. Whisper недетерминирован, повтор короткого чанка даёт реальный текст.

**ffmpeg не нужен** — всё через `hound::WavReader/WavWriter` из `Cargo.toml`.

### WHISPER_HALLUCINATION_PATTERN — ужатый regex

Было: `(?i)\s*[Сс]убтитр[ыа](\s+(создавал[аи]?|перевод[ил]*|by|сделал[аи]?))?[\s\S]*$` — `[\s\S]*$` мог съесть **произвольный хвост** после любого упоминания слова «субтитры», включая реальный контент. Суффикс был опциональным.

Стало: `(?i)\s*[Сс]убтитр[ыа]\s+(создавал[аи]?|перевод[ил]*|by|сделал[аи]?)[^\n]*$` — требуется конкретный суффикс («создавал/перевод/by/сделал»), удаление ограничено одной строкой. Случайно произнесённое «субтитры» внутри речи больше не тащит за собой весь конец.
